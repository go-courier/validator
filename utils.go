...
func RangeFromUint(min uint64, max *uint64) []*RuleLit {
	ranges := make([]*RuleLit, 2)

	if min == 0 && max == nil {
		return nil
	}

	ranges[0] = NewRuleLit([]byte(fmt.Sprintf("%d", min)))

	if max != nil {
		if min == *max {
			return []*RuleLit{ranges[0]}
		}
		ranges[1] = NewRuleLit([]byte(fmt.Sprintf("%d", *max)))
	}

	return ranges
}

func UintRange(tpe string, bitSize uint, ranges ...*RuleLit) (uint64, *uint64, error) {
